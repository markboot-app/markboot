    ...
    return Scaffold(
      ...
      body: Column(
        ...
        children: <Widget>[
          ...
          CommonWidget().commonTextField(controller: passwordCont,hintText: "Password"),
          CommonWidget().commonTextField(controller: confirmPassCont,hintText: "Confirm Password"),
          SizedBox(
            height: 20,
          ),
          Container(
  ...
  resetPasswordService() async {
    try {
      ...
      if(password != confirmPass) {
        Fluttertoast.showToast(msg: "Confirm Password does'nt match",
        backgroundColor: Colors.red,textColor: Colors.white
        );
        return;
      }

      await Firestore.instance.collection("Users").document("+91${widget.phoneNo}").setData(
        {
          "password" : password
        },merge: true
      );
      DocumentSnapshot snapshot = await Firestore.instance.collection("Users").document("+91${widget.phoneNo}").get();
