      ...
      children: <Widget>[
                ...
                onTap: (){
                 if(widget.path.contains("Internship")) {
                   Navigator.push(context, MaterialPageRoute(
                       builder: (context)=>InternshipUserListPage(docId:snapshot.documentID,internshipUserList: snapshot["appliedBy"] ?? new List(),)
                   ));
                 }
                 else if(widget.path.contains("Tournament")) {
                   Navigator.push(context, MaterialPageRoute(
                       builder: (context)=>TournamentAdminDetailsPage(taskUserList: snapshot["submittedBy"] ?? new List(),)
                   ));
                 }
                 else {
                   Navigator.push(context, MaterialPageRoute(
                       builder: (context)=>TaskUserListPage(taskUserList: snapshot["submittedBy"] ?? new List(),reward: snapshot["reward"],docId:snapshot.documentID)
                   ));
                 }
                },
                child: Container(
        ...
        Positioned(
          ...
          child: CircleAvatar(
            ...
            radius: 18,
            child: Text(widget.path.contains("Internship") ? (snapshot["appliedBy"]?.length?.toString()??"0") : (snapshot["submittedBy"]?.length?.toString()??"0"),
              style: TextStyle(
                  color: Colors.white,
                  fontSize: 15
              ),
              ...
              child: IconButton(
                ...
                onPressed: ()async{
                  try{
                    ...
                    await Firestore.instance.collection(widget.path).document(snapshot.documentID).delete();
